...
Heddle is version control for agent work. Agents don't work in commits, so we stop versioning their work like humans do. Every agent task — captures, retries, aborts, conflicts, merge — is bound into a single named, reviewable, recoverable thread. Drop it in beside Git. Keep Git. Add the missing control layer.
...
- **Web app** — closed source. The product surface at heddle.sh. Repository inspection, review, and admin woven over the hosted control plane.
- **Server** — closed source. The control plane behind the hosted app. Namespaces, repositories, grants, ingest, and review as first-class primitives.
